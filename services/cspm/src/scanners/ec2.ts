        ...
        if (dangerousOpen.length > 0) {
          ...
          findings.push(
            this.createFinding(
              ...
              `Security Group "${sgName}" (${sgId}) allows inbound traffic from 0.0.0.0/0 on: ${portSummary}.`,
              'HIGH',
              { sgId, sgName, openPorts: dangerousOpen },
              `Restrict inbound access in "${sgName}" to specific IPs or security groups for: ${portSummary}.`,
        ...
        if (permissiveEntries.length > 0) {
          ...
          findings.push(
            this.createFinding(
              ...
              `Network ACL "${naclId}" has ${permissiveEntries.length} rule(s) allowing traffic from 0.0.0.0/0: ${ruleSummary}.`,
              'MEDIUM',
              { naclId, permissiveEntries },
              `Review and restrict Network ACL rules in "${naclId}" to specific CIDR blocks.`,
        ...
        for (const instance of reservation.Instances || []) {
          ...
          if (instance.PublicIpAddress && instance.SecurityGroups?.length === 0) {
            findings.push(this.createFinding(
              ...
              `Instance "${instanceId}" has a public IP but no security group is attached`,
              'HIGH',
              { instanceId, publicIp: instance.PublicIpAddress },
              `Review and attach appropriate security group to instance "${instanceId}"`,
              ['ec2', 'public-ip', 'network']
            ));
          }
          ...
          if (instance.MetadataOptions?.HttpTokens !== 'required') {
            findings.push(this.createFinding(
              'IMDSv1 Enabled',
              `Instance "${instanceId}" does not enforce IMDSv2 for metadata access`,
              'HIGH',
              { instanceId, httpTokens: instance.MetadataOptions?.HttpTokens },
              `Enable IMDSv2 for instance "${instanceId}" to protect against SSRF attacks`,
              ['ec2', 'metadata', 'imds']
            ));
          }
          ...
          if (!instance.Monitoring?.State || instance.Monitoring.State === 'disabled') {
            findings.push(this.createFinding(
              ...
              `Instance "${instanceId}" does not have detailed CloudWatch monitoring enabled`,
              'LOW',
              { instanceId },
              `Enable detailed monitoring for instance "${instanceId}"`,
              ['ec2', 'monitoring']
            ));
          }
        }
      ...
      for (const instance of instances) {
        ...
        findings.push(
          this.createFinding(
            ...
            `Instance "${instanceName}" (${instanceId}) is internet-reachable. ` +
            ...
            `Verify "${instanceName}" needs to be publicly accessible. If not, remove the public IP ` +
              `or place it behind a load balancer. Restrict security group rules to specific CIDR ranges.`,
            ['ec2', 'network-reachability', 'public-exposure', 'internet-facing'],
          ),
        );
        ...
        for (const p of adminPorts) {
          ...
          findings.push(
            this.createFinding(
              `Sensitive Port ${p.port} (${label}) Exposed to Internet`,
              `Instance "${instanceName}" (${instanceId}) has ${label} port ${p.port} open to ` +
              ...
              `Restrict port ${p.port} in security group "${p.sgName}" immediately. ` +
                `Use a bastion host, VPN, or AWS Systems Manager Session Manager instead of direct ${label} access.`,
              ['ec2', 'network-reachability', 'open-port', 'internet-facing', label.toLowerCase()],
            ),
          );
        }
      }
    ...
    try {
      ...
      if (!result.EbsEncryptionByDefault) {
        findings.push(this.createFinding(
          ...
          'EBS default encryption is not enabled for this region. New EBS volumes will not be automatically encrypted.',
          'MEDIUM',
          { ebsEncryptionByDefault: false, region: this.client.getRegion() },
          'Enable EBS default encryption in the EC2 console (EC2 → Settings → Data protection) or via CLI: aws ec2 enable-ebs-encryption-by-default',
