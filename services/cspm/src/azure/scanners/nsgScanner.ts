          ...
          for (const portSpec of allPorts) {
            ...
            if (exposedSensitivePorts.length > 0) {
              ...
              findings.push(this.emit(
                ...
                {
                  message: `NSG "${nsgName}" rule "${rule.name}" allows inbound traffic from 0.0.0.0/0 on port(s): ${exposedSensitivePorts.join(', ')}. This exposes management/data plane services to the public internet.`,
