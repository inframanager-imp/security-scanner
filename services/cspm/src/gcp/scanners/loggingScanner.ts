        ...
        for (const required of REQUIRED_LOG_FILTERS) {
          ...
          if (!hasMetric) {
            findings.push(this.emit(
              ...
              { message: `Project "${project}" does not have a log-based metric for "${required.title}" (${required.cisControl}). Without this metric, changes to critical resources cannot trigger alerts.` },
            ));
          }
        }
