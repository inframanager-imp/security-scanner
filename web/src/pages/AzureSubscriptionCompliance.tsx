...
const FRAMEWORK_COLORS: Record<AzureFrameworkId, { borderLeft: string; bar: string }> = {
  CIS_AZURE: { borderLeft: 'border-l-blue-600',    bar: 'bg-blue-600'    },
  NIST:      { borderLeft: 'border-l-slate-600',   bar: 'bg-slate-600'   },
      ...
      {open && (
        <div className="px-4 pb-4 pt-2 border-t border-gray-100 bg-slate-50/50 space-y-3">
          ...
          {ctrl.findingTitles.length > 0 && (
            <div>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Mapped Findings</p>
              <div className="flex flex-wrap gap-1.5">
                {ctrl.findingTitles.map((title) => (
                  <span key={title} className="inline-flex items-center px-2 py-1 rounded-md text-xs bg-white border border-gray-200 text-gray-700 shadow-xs">
                    {title}
                  </span>
                ))}
              </div>
        ...
        <div className="border-t border-gray-100">
          <div className="p-4 space-y-2 max-h-[500px] overflow-y-auto">
            {visibleControls.length === 0 ? (
              ...
              visibleControls.map((ctrl) => <ControlRow key={ctrl.id} ctrl={ctrl} />)
            )}
          </div>
        </div>
      )}
...
export function AzureSubscriptionCompliance() {
  ...
  const subName = 'Azure Subscription';

  return (
    <div className="space-y-6">
      
      {/* Redesigned Header Row: Back Button -> Azure Logo -> Subscription Title & Metadata -> Overall Score Card */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-1">
        
        {/* Left Section: Back Arrow -> Azure Logo -> Title -> Details Subtitle */}
        <div className="flex items-center gap-3.5">
          {/* 1. Back Button */}
