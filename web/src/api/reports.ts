...
function transformReportHtml(html: string, provider: ReportProvider): string {
  let updated = html;
  ...
  // Inject top-right watermark for the company
  updated = updated.replace(
    /<div class="cover"[^>]*>/i,
    '$&\n  <div class="watermark-top-right"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg><div class="watermark-text">CLOUD<br/>SECURITY<br/>SCANNER</div></div>'
  );
...
<style>
  ...
  .stat-val.info { color: #64748b !important; }
  .gauge-grid { flex-wrap: nowrap !important; justify-content: space-between !important; gap: 12px !important; overflow: hidden !important; }
  .gauge { flex: 1 !important; max-width: 96px !important; }

  /* Cover overrides */
  .cover { position: relative !important; }
  .watermark-top-right { position: absolute !important; top: 24px !important; right: 32px !important; display: flex !important; flex-direction: column !important; align-items: center !important; gap: 6px !important; color: #64748b !important; }
  .watermark-text { font-size: 8.5px !important; font-weight: 800 !important; letter-spacing: 0.15em !important; text-transform: uppercase !important; line-height: 1.3 !important; text-align: center !important; }
  .watermark-top-right svg { width: 32px !important; height: 32px !important; stroke: #3b82f6 !important; }
...
export const reportsApi = {
  ...
  openVaptReport: async (provider: ReportProvider, targetId: string, filters?: VaptReportFilters): Promise<void> => {
    ...
    const transformedHtml = transformReportHtml(rawHtml, provider);
