...
import { Terminal, Shield, Play, Square, Settings, Trash2, Eye, Activity, X } from 'lucide-react';

interface DASTScannerProps {
  tenantId?: string;
}

export default function DASTScanner({ tenantId }: DASTScannerProps) {
  ...
  const handleDeleteJob = (jobId: string) => {
    ...
    aspmFetch(`/api/aspm/scans/jobs/${jobId}`, { method: 'DELETE' })
      ...
      .catch(err => console.error("Error deleting scan job:", err));
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', animation: 'fade-in 0.3s ease' }}>
      ...
      <div className="glass-panel" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '40px', color: 'var(--color-muted)' }}>
            Loading job registries...
          </div>
        ) : jobs.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--color-muted)', display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center' }}>
            <Shield size={36} style={{ opacity: 0.6 }} />
            <span>No scan jobs created yet. Click "+ Add Job" above to configure a job profile.</span>
          </div>
        ) : (
      ...
      {/* Add DAST Scan Job Wizard (Collapsible) */}
      {showWizard && (
        <div className="glass-panel" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '15px', animation: 'fade-in 0.2s ease' }}>
          ...
          <form onSubmit={handleCreateJob} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label style={{ fontSize: '0.8rem', color: 'var(--color-muted)', fontWeight: 500 }}>Target Endpoint</label>
              {tenantId ? (
                <input
                  type="text"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <label style={{ fontSize: '0.8rem', color: 'var(--color-muted)', fontWeight: 500 }}>Scan Engine Type</label>
              <select
                className="cyber-input"
                value={scanType}
                onChange={(e) => setScanType(e.target.value)}
              >
                <option value="DAST">DAST Vulnerability Scan (All Tools Consolidated)</option>
                <option value="API">API Security Sweep (IDOR & Rate Limit)</option>
              </select>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-glass)', paddingBottom: '12px', marginBottom: '15px' }}>
              <div>
                <h3 style={{ fontSize: '1.05rem', fontWeight: 700, color: 'var(--color-text)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  Job ID: <code>{activeJobForLogs.id}</code> Console Terminal
                </h3>
                <span style={{ fontSize: '0.75rem', color: 'var(--color-muted)' }}>
            ...
            <div style={{ 
              ...
              {logs.length === 0 ? (
                <div style={{ textAlign: 'center', color: 'var(--color-muted)', padding: '160px 0' }}>
                  ▋ Initializing terminal console pipelines...
                </div>
              ) : (
                logs.map((log, index) => {
                  let color = '#334155';
              ...
              {activeJobForLogs.status === 'Scanning' && (
                <div className="glow-primary animate-pulse" style={{ color: 'var(--color-primary)', marginTop: '5px' }}>
                  ▋ background scanner executing payloads...
                </div>
              )}
